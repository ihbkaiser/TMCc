...
class ECR(nn.Module):
    ...
    def forward(self, M):
        ...
        return loss_ECR
class TMSD7(nn.Module):
    def __init__(self, args):
        super(TMSD7, self).__init__()
        self.args = args
        ...
        mu2_np, var2_np = self.calculate_cluster_based_prior(args.num_topic, args.smoothing_count)
        self.mu2 = nn.Parameter(torch.tensor(mu2_np, dtype=torch.float32).to(args.device))
        self.var2 = nn.Parameter(torch.tensor(var2_np, dtype=torch.float32).to(args.device))
        self.mu2.requires_grad = False
        self.var2.requires_grad = False
    ...
    def calculate_cluster_based_prior(self,
                                  num_topics: int,
                                  smoothing_count: float = 0,
                                  epsilon: float = 1e-6
                                 ):
        """
        Compute the logistic-normal approximation to a Dirichlet(α) distribution.

        Given concentration parameters α = (α₁, …, α_K), this returns the mean vector μ
        and diagonal variance vector σ² of the equivalent logistic-normal:

        μ_k = log(α_k)
              - (1 / K) * sum_{i=1}^K log(α_i)

        σ_k² = (1 / α_k) * (1 - 2 / K)
              + (1 / K²) * sum_{i=1}^K (1 / α_i)
        
        Return Tuple[np.ndarray, np.ndarray]:
            - μ: Mean vector of the logistic-normal distribution.
            - σ²: Variance vector of the logistic-normal distribution.
        """
        labels = self.clusterinfo

        # 2) Đếm số doc theo từng chủ đề
        counts = np.zeros(num_topics, dtype=np.float32)
        for lab in labels:
            if 0 <= lab < num_topics:
                counts[int(lab)] += 1

        # 3) Làm mịn và chuẩn hóa
        smoothed = counts + smoothing_count
        total = smoothed.sum()
        avg = total / num_topics

        # a_k = (c_k / avg) + ε, đảm bảo ≥ ε
        a = (smoothed / (avg + epsilon)) + epsilon
        a = np.maximum(a, epsilon).reshape(1, -1)

        # 4) Chuyển sang mean Logistic-Normal: μ_k = log a_k − mean_j log a_j
        log_a = np.log(a)
        mu = (log_a.T - np.mean(log_a, axis=1, keepdims=True)).T

        # 5) Tính variance theo công thức xấp xỉ
        K = num_topics
        inv_a = 1.0 / a
        term1 = inv_a * (1.0 - 2.0 / K)
        term2 = (1.0 / (K * K)) * np.sum(inv_a, axis=1, keepdims=True)
        var = term1 + term2

        # 6) Trả về vector 1-D
        mu_prior = mu.squeeze().astype(np.float32)
        var_prior = var.squeeze().astype(np.float32)
    ...
    def forward(self, x, x_sub = None):
        ...
        dkm_loss = (p * distances).sum(dim=1).mean()
        loss = loss_TM + optimal_transport_loss + dkm_loss
