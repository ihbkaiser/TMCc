...
class SAE_NTM(nn.Module):
    def __init__(self, args):
        ...
        self.en1_units = self.sentence_dim  # Set to match sentence_dim
        self.fc11 = nn.Linear(args.vocab_size, self.en1_units)
        self.fc12 = nn.Linear(self.en1_units, self.en1_units)
    ...
    def forward(self, x, emb_batch):
        ...
        h_bow = self.fc1_dropout(e1)  # B x en1_units (which is sentence_dim)
        d_list = []
        for b in range(x.shape[0]):
            ...
            if m == 0:
                ...
                query = h_bow[b].unsqueeze(0)  # 1 x D
                key = h_sent  # m x D
